      ...
      <Card className="min-w-0 p-4 sm:p-5">
        <div>
          <h3 className="font-display text-base sm:text-lg font-semibold">
            Fund Rating & Consistency
          </h3>
          <p className="text-xs text-muted-foreground mt-0.5">
            Weighted score across rolling returns, drawdown, Sharpe, Sortino, volatility &
            consistency.
          </p>
        </div>
        <div className="mt-4 space-y-4">
          {ranked.map((s, rank) => {
            ...
            return (
              <div
                key={s.code}
                className="min-w-0 rounded-xl border border-border/60 bg-card/50 p-4"
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-display font-semibold text-gradient-brand num">
                      {s.overall}
                    </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="num font-medium">
                        {s.consistency}/100 · {consLabel.label}
                      </span>
                    </div>
...
function RankRow({
  ...
  label: string;
  fund?: Scored;
  value: (s: Scored) => string;
}) {
  if (!fund) return null;
  return (
    <div className="flex min-w-0 items-center justify-between gap-3 rounded-lg border border-border/60 bg-card/40 p-3">
        ...
        <div className="min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5 min-w-0">
            <span
              ...
              style={{ backgroundColor: colorFor(fund.i) }}
            />
            <span className="text-sm truncate">{fund.name}</span>
          </div>
        </div>
      </div>
      <div className="shrink-0 font-mono text-sm font-semibold text-gradient-brand">
        {value(fund)}
      </div>
    </div>
  );
